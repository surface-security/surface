...
class Test(TestCase):
    ...
    def setUp(self, now) -> None:
        ...
        responses.add(
            responses.GET,
            f"http://{settings.SCA_SBOM_REPO_URL}/v1/sbom/urn:uuid:46d764e2-aae1-4f82-b9f1-c616308e921d?vuln_data=True",
            status=200,
        ...
        responses.add(
            responses.GET,
            f"http://{settings.SCA_SBOM_REPO_URL}/v1/sbom/all?since={datetime.strftime(timezone.now() - timezone.timedelta(hours=1), '%Y-%m-%dT%H:%M:%S.%f')}",
            status=200,
            content_type="application/json",
