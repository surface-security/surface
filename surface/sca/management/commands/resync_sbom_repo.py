...
class Command(LogBaseCommand):
    ...
    def get_sboms(self, since: datetime) -> list[str]:
        ...
        res = requests.get(f"http://{settings.SCA_SBOM_REPO_URL}/v1/sbom/all", params={"since": since_str})
        res.raise_for_status()
        return res.json()

    def get_sbom_details(self, serial_number: str) -> dict[str, Any]:
        res = requests.get(f"http://{settings.SCA_SBOM_REPO_URL}/v1/sbom/{serial_number}", params={"vuln_data": True})
        res.raise_for_status()
        return res.json()
